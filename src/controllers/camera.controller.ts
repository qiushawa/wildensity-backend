...
export class CameraController extends CoordinatesController {
    ...
    async updateCamerasStatus(req: Request, res: Response): Promise<void> {
        const cameras = req.body; // [{cameraId, status, sdCardUsedSpace}, ...]
        const areaId = parseInt(req.params.areaId, 10);
        if (!Array.isArray(cameras) || cameras.length === 0) {
            return errorResponse(res, RESPONSE_CODE.BAD_REQUEST, "相機狀態更新列表是必需的");
        }

        // Update Camera && CameraStatusEvent
        ...
        // Create CameraStatusEvent records
        const statusEventPromises = await Promise.all(cameras.map(async ({ cameraId, status }) => {
            const lastEvent = await prisma.cameraStatusEvent.findFirst({
            where: {
                camera_id: cameraId,
                area_id: areaId,
            },
            ...
            return prisma.cameraStatusEvent.create({
                data: {
                camera_id: cameraId,
                area_id: areaId,
                status,
            ...
            return prisma.cameraStatusEvent.update({
                where: {
        ...
        try {
            await Promise.all([...updatePromises, ...statusEventPromises]);
            return successResponse(res, RESPONSE_CODE.SUCCESS, "相機狀態更新成功");
        } catch (error) {
